...
const items = ref<{ name: string; value: string | number }[]>([
  ...
  {
    name: '人数',
    value: `認証済み: ${asseptedMemberCount.value},  未認証:${
      MembersStore.members!.length - asseptedMemberCount.value
    },  計:${MembersStore.members?.length}`,
  },
  { name: 'マップ数', value: MapsStore.maps.length },
  { name: 'Todoリスト数', value: TodoListsStore.todoLists.length },
  {
    name: 'Todo数',
    value: TodoListsStore.todoLists.reduce((result, todoList) => {
