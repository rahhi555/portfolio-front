<template>
  <v-row>
    ...
    <app-btn color="error" class="px-2 ml-1" elevation="0" min-width="0" small text @click="deleteMap(map)">
      <v-tooltip bottom>
        <template #activator="{ on, attrs }">
      ...
      <v-card>
        ...
        <ValidationObserver v-slot="{ invalid }">
          ...
          <v-card-actions>
            ...
            <v-btn color="blue darken-1" :disabled="invalid" text @click="updateMap"> Update </v-btn>
          </v-card-actions>
        </ValidationObserver>
        <v-divider></v-divider>
      </v-card>
...
import { Map as MapParams } from 'interface'

const props = defineProps<{ map: MapParams }>()

const dialog = ref(false)

const deleteMap = async (map: MapParams) => {
...
const mapParams = Object.assign({}, props.map) as MapParams
reactive(mapParams)
watch(dialog, () => {
