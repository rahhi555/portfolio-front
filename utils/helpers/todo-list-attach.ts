...
export const attachTodoListStart = () => {
  isAttaching = true
}

export const attachTodoListEnter = (e: DragEvent) => {
...
export const attachTodoListLeave = () => {
  if (!isAttaching) return
  if (!SvgsStore.targetSvg) return
...
export const attachTodoListEnd = (e: DragEvent) => {
  ...
  SvgsStore.attachTodoList(todoListId).then(() => {
    window.removeEventListener('dragover', dragOverPreventDefault, false)
    isAttaching = false
    removeSvgTarget()
  })
