<template>
  <div>
    <svg  id="mysvg"
          ...
          width="100%"
          height="80vh"
          viewBow="0 0 100% 80vh"
          xmlns="http://www.w3.org/2000/svg"
          @mousemove="dragMiddle($event);resizeMiddle($event)"
          @mouseup="dragStop();resizeStop()" 
         >
      <template v-for="rect in rects" >
        <g :id="'rect-' + rect.id" :key="rect.id" :transform="'translate(' + rect.x + ',' + rect.y + ')'">
...
export default defineComponent({
  ...
  setup() {
    ...
    const setTargetingId = (e: SVGRectMouseEvent|SVGRectKeyboardEvent) => {
      ...
      if (!(parentSVG instanceof SVGGElement)) { return }
    ...
    const dragStart = (e: SVGRectMouseEvent): void => {
      setTargetingId(e)
      if (typeof(targetRect.value) === 'undefined') { return }
    ...
    // 変化する値は式に一つまでにしないと二次関数のような急勾配な変化具合になってしまう
    // 上への引っ張りはheightを増加させると同時にyを減少させる
    const resizeMiddleTop = (e: SVGRectMouseEvent) => {
      const resizeHeight = startHeight + startY - e.offsetY
      if (resizeHeight > 0) {
        targetRect.value!.height = Math.ceil(resizeHeight / MULTIPLE_NUMBER) * MULTIPLE_NUMBER
        targetRect.value!.y = Math.floor(e.offsetY / MULTIPLE_NUMBER) * MULTIPLE_NUMBER
      }
    }
    ...
    const resizeMiddleRight = (e: SVGRectMouseEvent) => {
      ...
      if (resizeWidth > 0) {
        targetRect.value!.width = Math.ceil(resizeWidth / MULTIPLE_NUMBER) * MULTIPLE_NUMBER
      }
    }
    ...
    const resizeMiddleLeft = (e: SVGRectMouseEvent) => {
      ...
      if (resizeWidth > 0) {
        ...
        targetRect.value!.width = Math.ceil(resizeWidth / MULTIPLE_NUMBER) * MULTIPLE_NUMBER
        targetRect.value!.x = Math.floor(e.offsetX / MULTIPLE_NUMBER) * MULTIPLE_NUMBER
      }
    }
    ...
    const nextId = computed<number>(() => {
      const ids = rects.value.map(rect => { return rect.id })
